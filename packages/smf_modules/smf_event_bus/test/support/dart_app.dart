...
/// real event_bus among them, so that the code of the events can be
/// analyzed and run with the Dart SDK alone.
///
/// event_bus is a Dart package without Flutter, and so is the code of the
/// events role. The other files of the app may need Flutter, or packages of
/// modules that this package does not depend on, so the analyzer checks the
/// files of the owners it is given, and a script that runs the app imports
/// none of the others. [delete] removes the directory.
final class DartApp {
  DartApp._(this._root, this._files);

  /// Writes the Dart files in `lib/` of [app].
  static Future<DartApp> write(RenderedApp app) async {
    final root = Directory.systemTemp.createTempSync('smf_event_bus_');
    final files = [
